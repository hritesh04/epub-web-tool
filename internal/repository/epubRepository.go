...
func (u *EpubRepository) Insert(ctx context.Context, data *model.Epub) (*model.Epub, error) {
	...
	row := u.db.QueryRow(ctx, "INSERT INTO epubs (id,title,size,translate_to,user_id,object_key,source,drive_link) VALUES($1,$2,$3,$4,$5,$6,$7,$8) returning id,title,size,translate_to,status,created_at;", data.Id, data.Title, data.Size, data.TranslateTo, data.UserID, data.ObjectKey, data.Source, data.DriveLink)
...
func (r *EpubRepository) AlreadyCompiling(ctx context.Context, id string) (*model.Epub, error) {
	...
	err := r.db.QueryRow(ctx, `
		...
		RETURNING id,object_key,source,drive_link;
	`, id).Scan(&epub.Id, &epub.ObjectKey, &epub.Source, &epub.DriveLink)
