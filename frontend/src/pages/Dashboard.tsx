...
} from "@/components/dashboard/BookCard";
...
const MAX_LINK_BYTES = 500 * 1024 * 1024; // 500MB
...
const mapTranslationJob = (data: any): TranslationJob => {
  return {
    ...
    status: (
      ...
      data.Status ||
      "queued"
    ).toLowerCase() as TranslationStatus,
...
export default function Dashboard() {
  const [translationLibrary, setTranslationLibrary] = useState<
    TranslationJob[]
  >([]);
  ...
  const [uploadFieldValues, setUploadFieldValues] =
    useState<UploadValues | null>(null);
  ...
  const validateFile = useCallback((file: File) => {
    if (
      !(file.name.endsWith(".epub") || file.type === "application/epub+zip")
    ) {
  ...
  const startUpload = useCallback(async () => {
    ...
    form.append(
      "X-Amz-Algorithm",
      uploadFieldValues?.["X-Amz-Algorithm"] ?? ""
    );
    form.append(
      "X-Amz-Credential",
      uploadFieldValues?.["X-Amz-Credential"] ?? ""
    );
    form.append("X-Amz-Date", uploadFieldValues?.["X-Amz-Date"] ?? "");
    form.append(
      "X-Amz-Signature",
      uploadFieldValues?.["X-Amz-Signature"] ?? ""
    );
  ...
  const startDriveImport = useCallback(async () => {
    const trimmedUrl = linkUrl.trim();
    if (!trimmedUrl || !targetLanguage) return;
    setDriveImporting(true);
    setUploadError(null);
    try {
      const res = await api.post("/upload/drive", {
        driveLink: trimmedUrl,
        title: "Imported Book",
        translateTo: targetLanguage,
      });
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground max-w-lg">
              Manage your translations, track progress, and download your
              polished books from one central workspace.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              size="lg"
              className="h-11 px-6 rounded-xl shadow-lg shadow-primary/20 gap-2"
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border/50 h-24 flex items-center p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold">
                  {
                    translationLibrary.filter((b) => b.status === "in-progress")
                      .length
                  }
                </div>
                <div className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border/50 h-24 flex items-center p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold">
                  {
                    translationLibrary.filter((b) => b.status === "completed")
                      .length
                  }
                </div>
                <div className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
          ...
          {epubsLoading ? (
            ...
            >
              ...
              <p className="text-muted-foreground max-w-sm mb-8">
                Your library is currently empty. Upload your first EPUB to start
        ...
        <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none bg-card/60 backdrop-blur-2xl">
          ...
          <div className="p-6 pt-0 space-y-6">
            ...
            {!uploading && !driveImporting && (
              <div className="grid grid-cols-2 gap-1 p-1 rounded-xl bg-background/50 border border-border/50">
                <button
                  type="button"
            ...
            {uploadMode === "link" ? (
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Paste a public Google Drive share link to an EPUB. Files up
                    to {MAX_LINK_BYTES / (1024 * 1024)}MB are supported.
                  </p>
                </div>
                ...
                <Button
                  ...
                  disabled={
                    !linkUrl.trim() || !targetLanguage || driveImporting
                  }
                >
                  {driveImporting ? (
              ...
              <div className="space-y-6">
                ...
                <div
                  ...
                  onClick={() =>
                    targetLanguage && fileInputRef.current?.click()
                  }
                >
                  <input
                    ref={fileInputRef}
              ...
              <div className="space-y-6">
                <div className="relative p-4 rounded-2xl border border-border/50 bg-background/50 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm truncate uppercase tracking-tight">
                      {selectedFile.name}
                    </p>
                    <p className="text-xs text-muted-foreground">
